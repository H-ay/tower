...
{   QList<WayPoint*> waypoint1;
    QString num1=num;
    QString files=":/config/map"+mapnumber+"/Points"+num1+".plist";
    QFile file(files);
    if (!file.open(QFile::ReadOnly | QFile::Text))
    {
...
void map::preLoadWavesInfo()
{   QString files=":/config/map"+mapnumber+"/Waves.plist";
    QFile file(files);
    if (!file.open(QFile::ReadOnly | QFile::Text))
    {
       // QMessageBox::warning(this, "TowerDefense", "Cannot Open TowersPosition.plist");
